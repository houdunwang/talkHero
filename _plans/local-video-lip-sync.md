...
- 当前阶段：阶段 0 CosyVoice2 最小资源链与三后端可行性门；Apple Silicon CPU 已完成固定模型的离线加载和短句技术出波形，生产依赖锁、获授权音色盲听、Windows CUDA/XPU、MuseTalk 视频链、质量时间轴、基础字幕和平台提交仍未完成。
...
- Red/Green 7（2026-09-09）：新增资源许可证商业使用/再分发硬门禁与自然语气唯一契约；Red 时安装器接受 `commercialUse: false` 且 voice 接受 `enthusiastic`，2 项按预期失败。Green 后 `pnpm exec vitest run apps/inference apps/voice` 为 12 files / 48 tests 通过；音色档案版本固定为 `cosyvoice2-0.5b`，合成同时传入本地参考音频与转写。
- CosyVoice2 固定链探针（2026-09-09）：模型 revision `eec1ae6c79877dbd9379285cf8789c9e0879293d` 的 11 个最小模型文件逐项下载并复算 SHA-256，与审计值一致；新增只含推理对象的固定 YAML。上游 Matcha 推理导入会经 `utils/__init__` 误引 Hydra/Lightning，适配器以锁定 commit 的窄兼容层跳过训练初始化。PyPI WeTextProcessing 默认仍访问 ModelScope，因此改用 Apache-2.0 派生最小 TN 适配和固定 FST commit `dba0737b6bc1093e108eb5bc512babf4a7e157b4` 的四个本地文件；从隔离环境卸载 `wetext`/ModelScope 后，在不可连接代理下完成 CPU 加载、中英文数字规范化和 2 秒短句出波形。加载 11.82 秒、含生成总计 17.98 秒，峰值约 6.1GB；上游演示音频只作技术通路证据，不作商业质量验收。
- 本轮门禁（2026-09-09）：目标回归 `pnpm exec vitest run apps/inference apps/voice` 为 12 files / 48 tests，全量 `pnpm test` 为 23 files / 107 tests，均通过；`pnpm typecheck`、`pnpm build`、三个 Worker Python 文件的 3.11 `py_compile` 与 `git diff --check` 通过；`pnpm lint` 0 error，仅保留 `apps/core/renderer/routes/config.tsx:80` 的既有 Prettier warning。构建仍保留 auth store 动态/静态导入的既有 Vite warning。
- MuseTalk 固定链探针（2026-09-09）：新增固定资源审计，纠正“模型也是 MIT”的过度概括：代码为 MIT，模型卡元数据为允许附条件商业使用/再分发的 CreativeML OpenRAIL-M，正式包须附完整条款和使用限制。固定 UNet、SD VAE safetensors、Whisper tiny safetensors 与 YuNet 均建立 revision/hash；`unet.pth` 以 `weights_only=True` 读取为 686 个纯 Tensor 并 strict 装入 849947844 参数 UNet。macOS CPU 的 UNet、VAE、Whisper 关键前向与不可连接代理离线加载通过，峰值分别约 7.28GB、4.25GB；这些仅为算子证据，不开放 CPU 视频。MIT YuNet 连续 100 帧技术检测约 80fps，作为移除 S3FD、BiSeNet、SFace 和 DWPose/mmpose 的主体锁定/保守嘴部蒙版候选，仍须获授权多人/走动样片验证。
- Red/Green 8（2026-09-09）：先新增 YuNet 主体连续匹配、低置信/歧义失败和侧脸嘴部区域测试，Red 因 `tracking-policy` 不存在正确失败；Green 后 1 file / 3 tests 通过。当前只形成纯策略，不把上游互联网测试视频当身份/画质验收，也不解除 Worker 的 `video.lipsync` fail-closed。
- Apple Silicon MPS 关键性能（2026-09-09）：PyTorch 2.3.1 虽可运行同一 MuseTalk FP16 UNet，但热身后约 0.83 秒/帧，淘汰为 Mac runtime 候选；独立 PyTorch 2.14.0 在禁止 CPU fallback 时热身后 UNet 约 0.11～0.13 秒/帧，VAE encode×2 + UNet + decode 核心链约 0.384 秒/帧，batch 8 约 0.374 秒/帧。按 4500 帧机械外推约 28～29 分钟且未计完整管线，说明 MPS 可行但仍慢；后续只评估固定新版 runtime、latent 缓存/流水线和必要 Core ML 探针，不降低模型或画质门禁，也不把外推写成产品承诺。
