...
当前实现要求用户授权确认，通过一次性文件令牌把 B 视频交给 Worker，限制 10～60 秒和单音轨，使用 FFmpeg 提取参考音频、faster-whisper 转写，并以原子目录保存参考、转写和特征摘要；原始 B 视频不会被复制到音色目录。音色支持本地试听、重命名、原子更新和删除；仓库将读取、崩溃恢复与写入统一串行，唯一 revision 标识决定更新恢复方向，并用引用计数阻止仍被任一生成任务使用的音色更新或删除。新建和更新档案固定为 `cosyvoice2-0.5b`，旧 IndexTTS 档案不迁移且不能被新模型静默复用。CosyVoice2 首版只开放自然语音和 0.8～1.2 语速，生成时同时传入参考音频及其本地转写，输出原子提交并通过受管只读 URL 试听。可靠的多人说话检测和各目标平台真实质量验证仍未完成。
